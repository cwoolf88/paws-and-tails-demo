#!/usr/bin/env node
/**
 * Builds the local next-address-server-js checkout and reinstalls it in this demo.
 *
 * Usage:
 *   npm run sdk:sync:local
 *   LOCAL_NEXT_ADDRESS_SERVER_JS=../other/next-address-server-js npm run sdk:sync:local
 */
...
execSync("npm run install:sdk:local", {
  cwd: projectRoot,
  stdio: "inherit",
  env: { ...process.env, LOCAL_NEXT_ADDRESS_SERVER_JS: raw },
});
